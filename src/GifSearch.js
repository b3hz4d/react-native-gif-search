...
class GifSearch extends PureComponent {
  constructor(props) {
    ...
    if (props.provider != null) {
      if (
        props.provider == providers.TENOR ||
        props.provider == providers.GIPHY
      ) {
    ...
    this.tenorMediaFilter = this.calcTenorMediaFilter(
      this.tenorGifPreview,
      this.tenorGifSelected
    );
  ...
  calcTenorMediaFilter = (tenorGifPreview, tenorGifSelected) => {
    if (tenorGifPreview == "mediumgif" || tenorGifSelected == "mediumgif") {
      return null;
    }
    if (tenorGifPreview == "nanogif" || tenorGifSelected == "nanogif") {
      return "basic";
    }
    return "minimal";
  };
  ...
  handleRequestResponses = (tenorResponseJSON, giphyResponseJSON) => {
    if (tenorResponseJSON == null) {
      tenorResponseJSON = { results: [] };
    }
    if (giphyResponseJSON == null) {
      giphyResponseJSON = { data: [] };
    }

    if (
      tenorResponseJSON.results.length > 0 ||
      giphyResponseJSON.data.length > 0
    ) {
      var tenor_gifs = this.addProviderAndTypeInfo(
        tenorResponseJSON.results,
        providers.TENOR
      );
      var giphy_gifs = this.addProviderAndTypeInfo(
        giphyResponseJSON.data,
        providers.GIPHY
      );
      var all_gifs = this.joinAndRemoveDuplicateIds(
        this.state.gifs,
        this.interleaveGifArrays(tenor_gifs, giphy_gifs)
      );
      this.setState({
        gifs: all_gifs,
      ...
      this.setState({
        fetching: false,
        gifsOver: true,
        noGifsFound: this.state.gifs.length == 0,
      });
    }
  };
  ...
  showGifs = (endpoint) => {
    ...
    if (this.state.currentProvider == providers.ALL) {
      this.fetchTenorGifs(endpoint)
        .then((tenorResponseJSON) => {
          this.fetchGiphyGifs(endpoint)
            .then((giphyResponseJSON) => {
              if (search_term == this.state.search_term) {
                this.handleRequestResponses(
                  tenorResponseJSON,
                  giphyResponseJSON
                );
              }
  ...
  fetchTenorGifs = (endpoint) => {
    var limit = Math.min(
      this.gifsToLoad,
      this.maxGifsToLoad - this.state.offset
    );
    if (this.state.currentProvider == providers.ALL) {
    ...
    if (endpoint == endpoints.TRENDING) {
      return Requests.fetch("GET", TENOR_BASE_URL + "trending", {
        ...
        ...(this.tenorMediaFilter != null && {
          media_filter: this.tenorMediaFilter,
        }),
  ...
  joinAndRemoveDuplicateIds = (old_array, new_array) => {
    var new_array_unique = new_array.filter((obj1, index1) => {
      return !new_array.some(
        (obj2, index2) => obj1.id === obj2.id && index1 != index2
      );
    }); // remove duplicates
    new_array_unique = new_array_unique.filter((o1) => {
      return !old_array.some((o2) => o1.id === o2.id);
    }); // remove duplicates with previous gifs
  ...
  interleaveGifArrays = (tenor_array, giphy_array) => {
    if (tenor_array.length == 0) {
      return giphy_array;
    }
    if (giphy_array.length == 0) {
      return tenor_array;
    }

    var result = [];
    var i,
      l = Math.min(tenor_array.length, giphy_array.length);
  ...
  showStickersButtonPressed = () => {
    if (this.state.currentGifType != gif_types.STICKER) {
      this.setState(
        {
          currentGifType: gif_types.STICKER,
          currentProvider: providers.GIPHY,
          offset: 0,
          gifs: [],
          tenorGifs: [],
          giphyGifs: [],
  ...
  componentDidMount() {
    ...
    if (this.numColumns > 1) {
      Dimensions.addEventListener("change", () => {
        this.setState({
          gifSize: Dimensions.get("window").width / this.numColumns - 15,
        });
      });
    }
  }
  ...
  render() {
    return this.props.visible == null || this.props.visible ? (
      <View style={[this.styles.view, this.props.style]}>
        ...
        >
          <TextInput
            placeholder={
              this.state.currentGifType == gif_types.GIF
                ? this.placeholderText
                : this.stickersPlaceholderText
            }
        ...
        {this.gifType == gif_types.ALL ? (
          ...
          >
            <TouchableOpacity
              style={
                this.state.currentGifType != gif_types.GIF
                  ? [this.styles.gifTypeButton, this.props.showGifsButtonStyle]
                  : [
                      this.styles.gifTypeButton,
                      this.props.showGifsButtonStyle,
                      this.styles.gifTypeButtonSelected,
            ...
            >
              <Text
                style={
                  this.state.currentGifType != gif_types.GIF
                    ? [
                        this.styles.gifTypeButtonText,
                        this.props.showGifsButtonTextStyle,
                      ]
                    : [
                        this.styles.gifTypeButtonText,
                        this.props.showGifsButtonTextStyle,
                        this.styles.gifTypeButtonSelectedText,
                        this.props.showGifsButtonSelectedTextStyle,
                      ]
                }
              >
            ...
            <TouchableOpacity
              style={
                this.state.currentGifType != gif_types.STICKER
                  ? [
                      this.styles.gifTypeButton,
                      this.styles.stickerTypeButton,
                      this.props.showStickersButtonStyle,
                    ]
                  : [
                      this.styles.gifTypeButton,
                      this.styles.stickerTypeButton,
                      this.props.showStickersButtonStyle,
                      this.styles.gifTypeButtonSelected,
                      this.props.showStickersButtonSelectedStyle,
                    ]
              }
              ...
              <Text
                style={
                  this.state.currentGifType != gif_types.STICKER
                    ? [
                        this.styles.gifTypeButtonText,
                        this.props.showStickersButtonTextStyle,
                      ]
                    : [
                        this.styles.gifTypeButtonText,
                        this.props.showStickersButtonTextStyle,
                        this.styles.gifTypeButtonSelectedText,
                        this.props.showStickersButtonSelectedTextStyle,
                      ]
                }
              >
          ...
          renderItem={({ item, index }) => {
            ...
            if (item.provider == providers.TENOR) {
              ...
              if (parseInt(item.images[this.giphyGifSelected].height)) {
                aspect_ratio =
                  parseInt(item.images[this.giphyGifSelected].width) /
            ...
            return (
              <TouchableOpacity
                activeOpacity={0.7}
                onPress={() => {
                  this.props.onGifSelected(gif_better_quality, item);
                  Keyboard.dismiss();
                }}
                onLongPress={() => {
              ...
              >
                <Image
                  ...
                  style={[
                    this.styles.image,
                    !this.horizontal && (index + 1) % this.numColumns === 0
                      ? this.styles.lastColumnImage
                      : {},
                    this.numColumns > 1
                      ? {
                          width: this.state.gifSize,
                          minHeight: this.state.gifSize,
                          maxHeight: this.state.gifSize,
                        }
                      : {
                          aspectRatio: aspect_ratio ? aspect_ratio : 4 / 3,
                          height: 150,
                        },
                    this.props.gifStyle,
                  ]}
          ...
          ListFooterComponent={
            this.state.offset < this.maxGifsToLoad && !this.state.gifsOver ? (
              ...
              >
                <Text
                  style={[
                    { textAlign: "center", fontSize: 20, color: "grey" },
                    this.props.noGifsFoundTextStyle,
                  ]}
                >
                  {this.noGifsFoundText}
                </Text>
              </View>
            ) : null
          }
